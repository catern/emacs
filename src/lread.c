   ...
   Lisp_Object nosuffix, Lisp_Object must_suffix)
{
  ...
  if (SCHARS (file) == 0)
      ...
      else
	{
      ...
      if (FUNCTIONP (Vload_filter_path_function))
	load_path = calln (Vload_filter_path_function, load_path, file, suffixes);
...
through `require'.  */);
  ...
  DEFVAR_LISP ("load-filter-path-function",
	       Vload_filter_path_function,
...
`load-path'.
*/);
  Vload_filter_path_function = Qnil;
