...
(defvar load-filter-path--cache nil
  "A cache used by `load-filter-path-cache-directory-files'.
...
(defun load-filter-path-cache-directory-files (path file suffixes)
...
Caches directory contents in `load-filter-path--cache'."
  (if (file-name-directory file)
      ;; FILE has more than one component, don't bother filtering.
      path
    (seq-filter
     (let ((rx-and-ht
            (with-memoization (alist-get suffixes load-filter-path--cache nil nil #'equal)
              (cons
