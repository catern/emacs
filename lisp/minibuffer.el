            ...
            (let ((comps ()))
              ...
              (let* ((prefix (try-completion fixed comps))
                ...
                (if unique
                  ...
                  (when (cl-some (lambda (elem) (eq elem 'prefix)) wildcards)
                    (setq prefix fixed))
                  (push prefix res)
                  ...
                  (when (and (cl-some (lambda (elem) (memq elem '(star point prefix))) wildcards)
                    ...
                    (let ((suffix
                      ...
                      (unless (equal suffix "")
                        (push suffix res))))
                  ;; We pushed these wildcards on RES, so we're done
                  ;; with them.
                  (setq wildcards nil))
                (setq fixed "")))))
